...
以真实机器人联调确认（2026-09-06），以下是文档只用文字描述、需要实测才能定形的四点：

| 观测项 | 实测值 | 对平台的影响 |
|---|---|---|
| `from.userid` | `T31560051A` 形态，**非明文 userid** | 文档所述「创建者非超管时为企业主体下的加密 userid」成立。平台无需特殊处理：`channel_users` 本就负责外部标识到内部用户的映射，且 `uk_scope` 以 `channel_binding_id` 起头，加密标识天然隔离在该绑定内 |
| 单聊的 `chatid` | **不返回** | `scope_key` 取 `from.userid`，与 `chattype` 分支一致 |
| `body.msgid` | `dd48d208ffdb5a6fdf0883ac82846fff` | 明文可读，可直接作幂等键，无需先解密 |
| `headers.req_id` | `7PtkttaERASl-Ec3AMD1ygAA` | 回复透传该值被平台接受。这坐实了它必须穿过队列到 Worker 再穿回来，不能重新生成 |

另外两点被真实端点验证：

- **`stream` + `finish=true` 一次发完是有效用法。** 本平台不交付流式响应，用的是流式消息的退化写法。真实端点接受，该映射成立，无需为「非流式」单开一种消息类型。
- **取消后 `Run()` 正常返回。** 真实连接上验证了心跳与读循环的生命周期绑定（见 §9.4 与完成度台账 §八 #9 记录的死锁修复）。

未经实测的部分：群聊（`chatid` 场景）、多媒体 `aeskey` 解密、事件回调的 5 秒窗口、限频实际表现。
...
**但「已入队」不等于「已投递」**，这是长连接对状态机提出的新要求。

回调模式下 `deliver` 返回即已送达，二者等价。长连接打破了这个等价：返回只表示回执已交给出站信箱，真正的发送在持连副本，**且只有那个副本知道结果**。若在交接时就置 `succeeded`，则 socket 随后失败时行已是终态，无人重投——回复静默丢失，而 Gateway 在听、Worker 在消费、存储都通,所有信号皆绿。

因此状态推进的责任分给了两个进程：

```text
Worker    执行完成 → 写出站信箱 → 行保持 processing（不置终态）
持连副本  取出回执 → 经 socket 发出 → 置 succeeded 或 delivery_failed
```

保持 `processing` 而非引入新状态，是因为它恰好复用了既有语义:这与「滞留请求」是同一状态,于是持连副本在发送前崩溃时,现成的 Sweeper 扫描就能捞回,不需要额外机制。

`Worker.deliver` 的签名相应从 `error` 改为 `(queued bool, err error)`——把「交接」与「送达」的区别抬到类型层面，让调用方无法沿用旧假设而不自知。
