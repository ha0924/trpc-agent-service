...
源文件见 [`assets/系统架构图.html`](assets/系统架构图.html)（浏览器直接渲染），矢量版见 [`assets/系统架构图.svg`](assets/系统架构图.svg)。
...
核心时序图见 [`assets/核心时序图.html`](assets/核心时序图.html)（HTML + Mermaid，浏览器直接渲染）。
...
完整表清单（26 张）、通用约定与已落地 12 张表的 DDL 见 [`数据模型设计.md`](数据模型设计.md)，本节只列核心实体。
