...
完成度相关有两份，切分维度不同、不可互相替代：台账按**要求条目**记，功能清单按**功能模块**记。冲突时以台账为准，例外须在功能清单里显式标注。
...
| `docs/完成度台账.md` | 完成度的唯一事实来源，按 README 五大块记「做到什么程度」 |
| `docs/功能清单.md` | 需求理解与应做清单，按功能模块记「一共有哪些功能」 |
| `docs/功能开发计划.md` | 范围裁剪与开发顺序 |
| `docs/功能完成记录.md` | 每轮做了什么、验证强度多高 |
| `docs/待验证清单.md` | 已实现未实测的部分与人工命令 |
